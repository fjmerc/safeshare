...
func (r *UserRepository) RegenerateClaimCode(ctx context.Context, fileID, userID int64) (*repository.ClaimCodeRegenerationResult, error) {
	...
	newClaimCode, err := generateUniqueClaimCode(ctx, tx, fileID)
	if err != nil {
		return nil, err
	}
...
// generateUniqueClaimCode generates a unique claim code with exponential backoff.
// Returns ErrServiceUnavailable if a unique code cannot be generated.
func generateUniqueClaimCode(ctx context.Context, tx *sql.Tx, fileID int64) (string, error) {
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		var exists bool
		err = tx.QueryRowContext(ctx, "SELECT EXISTS(SELECT 1 FROM files WHERE claim_code = ?)", code).Scan(&exists)
		if err != nil {
