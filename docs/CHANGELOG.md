...
- Malware scanning is now effective. Previously, when encryption was enabled, uploads were scanned as ciphertext (which never matches virus signatures, so every file was reported clean); scanning also ran asynchronously, so a file could be downloaded before its verdict was known. Scanning now runs on the plaintext, synchronously, before the file's claim code becomes retrievable, on both the direct and chunked-upload paths. Inconclusive scans (scanner errors or unrecognized responses) are now treated as not-clean instead of clean. Infected uploads are recorded, purged from disk, and reported via webhook, and are blocked at download. In addition, downloads of any file not affirmatively confirmed clean are refused by default (425 while a scan is pending, 403 otherwise); to treat scanning as advisory and allow files whose scan errored, disable "Block Downloads Until Scan Is Clean" on the admin Enterprise Features page (or set `MALWARE_SCAN_BLOCK_UNTIL_CLEAN=false` as the initial default before any settings are saved) — note the gate also applies to files uploaded before scanning was enabled. A `safeshare_malware_scans_total` metric reports scan verdicts. Requires the malware-scanning feature and a reachable ClamAV instance.
...
- Fixed a data race on webhook delivery where the shared `*http.Client`'s `Timeout` field was mutated on every call from concurrent worker goroutines. Each delivery now builds a fresh `*http.Client` wrapping a shared `*http.Transport`, so the connection pool and SSRF guards are reused but per-call fields cannot race. Previously, a webhook configured for a 5-second timeout could transiently inherit a 60-second timeout from another in-flight delivery (and vice versa), causing premature cancellation or hung deliveries that piled up retries.
...
- File-download passwords are now sent in the `X-File-Password` header or a POST form body instead of the URL query string, so the password no longer leaks into reverse-proxy access logs (Cloudflare, Traefik, nginx), browser history, or outbound Referer headers on the download landing page. The legacy `?password=…` query-string form is still accepted for backwards compatibility but now emits `Deprecation: true`, `Sunset: 2026-09-30`, and `Referrer-Policy: no-referrer` headers and is logged at WARN with a Prometheus counter (`password_via_query_deprecated`). Web UI and the resumable downloader switched to header transport; the cross-origin `<a>` tag download fallback is the only remaining query-string path and will move to a signed-token exchange in `v1.6.0`. The `/api/claim/{code}` endpoint also accepts POST now (capped at 4 KiB body) for programmatic clients that can't set custom headers.
...
- Hardened the chunked-upload assembly handler against authenticated-user denial-of-service. Previously `/api/upload/complete/{id}` spawned an assembly goroutine *before* acquiring its concurrency slot, so a malicious client could spam the endpoint to accumulate unbounded goroutines and memory (each carrying a partial-upload copy in closure). The handler now acquires a slot non-blockingly on the request goroutine *before* doing any expensive precondition work (filesystem stats, chunk-integrity hashing, disk-space probe, DB lock) and returns `503 Service Unavailable` with `Retry-After: 5` and code `ASSEMBLY_BUSY` if the assembly pool is saturated. The DB processing lock is released cleanly via a new conditional `ReleaseProcessingLock` repository method (does not clobber rows that have already advanced to `completed` or `failed`). The endpoint is also now subject to the same rate limit as chunk uploads (10× the configured upload limit). Pool capacity is configurable via `ASSEMBLY_WORKERS_MAX` (default 10); a new metric `safeshare_chunked_upload_assembly_saturated_total` counts saturation refusals for operator alarms.
- **At-rest encryption format bump SFSE1 → SFSE2** (SH-2.1, ADR-011). New uploads are encrypted in a new on-disk format that binds per-chunk Additional Authenticated Data (`enc_file_id ‖ chunk_index ‖ is_last`) into every AES-GCM tag, plus a `total_plaintext_length` trailer in the header. This defeats four classes of tamper against the encrypted blob that an attacker with storage-write access (compromised S3 bucket, malicious backup operator, ransomware, supply-chain compromise of the storage layer) could previously execute silently: truncate-trailing-chunks, reorder, cross-file splice (between two files encrypted with the same key), and total-plaintext-length forgery (including the "zero-byte collapse" attack where a non-empty file is truncated to header-only and the length trailer rewritten to zero). Full-file downloads also verify the plaintext SHA-256 against `files.checksum_sha256` after decrypt. Operator action required: none — legacy SFSE1 files remain readable indefinitely via version-byte dispatch; the format upgrade applies to new uploads and to files re-encrypted by the metadata-strip pipeline. A schema migration adds `files.enc_file_id` (16-byte random per-file identifier; SQLite `BLOB`, PostgreSQL `BYTEA`). An opt-in `cmd/migrate-encryption --upgrade-format` flag for rotating legacy files in place is planned for a follow-up PR.
- **S3 encrypted storage now streams end-to-end** (SH-2.2). The S3 backend's `Retrieve`, `StreamRange`, `Store`, and `AssembleChunks` no longer buffer the full ciphertext or plaintext in heap. Previously a 1 GB S3 download spiked ~2 GB of heap (separate ciphertext + plaintext buffers) and five concurrent downloads OOM-killed a typical container; under the new path a 1 GB end-to-end round-trip uses < 40 MB of heap delta (measured via `runtime.MemStats.HeapInuse` in the new MinIO integration test). Range requests now issue two S3 GetObject calls per request (one for the 18-byte SFSE2 header, one for the covering ciphertext range computed via the new `ComputeSFSE2CiphertextRange` helper) instead of fetching and decrypting the whole object. The S3 wrapper now emits SFSE2 (formerly SFSE1) and stashes the SFSE2 `enc_file_id` + plaintext length in S3 object UserMetadata so the wrapper is self-contained — no DB plumbing is required for the wrapper itself to decrypt files it wrote. Legacy SFSE1 objects remain readable via version dispatch. Operator action required: none for filesystem deployments; S3-backed deployments get the fix automatically the next time they upgrade. Note: S3 UserMetadata is not cryptographically protected, so a write-access attacker who can substitute an entire object body + metadata together can replace files (see `docs/SECURITY.md`); the AAD still defeats every in-place tamper class. SH-3.5 (ADR-013) closes that gap when callers pass DB-sourced expectations via the new `IntegrityVerifyingBackend` sub-interface.
- **Removed the dead local-filesystem encryption wrapper** (`internal/storage.EncryptedStorage`, SH-2.2). Production upload/download handlers have always called `utils.EncryptFileStreamingV2*` / `utils.DecryptFileStreamingRangeAny` against raw filesystem paths directly, never through the wrapper's `Store` / `Retrieve` / `StreamRange` / `AssembleChunks`. The wrapper layer was reached only for its `HealthCheck` passthrough. Deleting it (727 lines of duplicated SFSE1 emit/decrypt code, plus 506 lines of V1-only tests) eliminates a latent code-duplication bug source. No behaviour change for end users; encryption-at-rest is unchanged.
- **S3 storage backend now detects object-substitution attacks PRE-STREAM** (SH-3.5, ADR-013). Adds a new `IntegrityVerifyingBackend` sub-interface with `RetrieveWithExpected` and `StreamRangeWithExpected` methods. Both accept three DB-sourced expectations — `expectedEncFileID`, `expectedSHA256Hex`, `expectedPlaintextLen` — and enforce them in layered order so that **zero attacker-controlled plaintext bytes ever flow to the caller** on detection: (1) the wrapper cross-checks the metadata-supplied `enc_file_id` against the DB-sourced value BEFORE launching the decrypt goroutine; mismatch → synchronous error, pipe never opens; (2) the cross-checked `enc_file_id` is then used as per-chunk AAD input, so a body-only substitution (metadata kept, ciphertext replaced with another same-key file's) fails on the first chunk's AAD verification before any plaintext is written; (3) the SFSE2 header trailer is cross-checked against the DB-sourced `plaintext_len`; (4) on full-file reads the post-decrypt SHA-256 is verified as belt-and-suspenders against the out-of-scope "attacker also has the encryption key" case. Closes the known limitation documented in the SH-2.2 entry above: a write-access attacker can no longer silently replace one stored file's body + UserMetadata with another same-key file's, regardless of whether the caller requests a full retrieve or a partial Range. Legacy `Retrieve` / `StreamRange` are preserved (`// Deprecated:` doc-tagged) for backward compatibility — admin recovery tools and health probes that genuinely have no DB context to source expectations from. Filesystem backend implements the sub-interface as a trivial pass-through (filesystem bytes share a trust boundary with the DB row in a normal SafeShare deployment); the pass-through is explicitly documented as unsafe for shared/network filesystems where the trust-boundary assumption breaks. Legacy SFSE1 objects cannot honour the new expectations (no AAD, no length trailer); callers passing non-sentinel expectations against an SFSE1 object now receive a clear error pointing at `migrate-encryption --upgrade-format` rather than silently returning unverified bytes. Handler integration — wiring `STORAGE_TYPE=s3` deployments to call the verifying methods — is intentionally deferred to a follow-up PR so this change ships as preparatory API plumbing without behaviour changes to the filesystem default. Operator-level mitigations (S3 object versioning + MFA-delete, separate IAM credentials for DB and S3) remain recommended for defence-in-depth.
- **Download counter no longer consumed by pre-fetches and partial-Range probes** (SH-2.3, ADR-012). The download cap for `max_downloads=N` files used to be enforced by incrementing `download_count` *before* any bytes were streamed. A browser pre-fetch, link-preview crawler, mid-stream network blip, or attacker who guessed the claim code and sent a 1-byte `Range: bytes=0-0` probe could silently burn the only download of a single-use file and lock the legitimate recipient out. The handler now uses a two-phase reservation pattern: `ReserveDownload` takes an in-flight slot (preserving the original P1 race-fix property: concurrent claims still admit exactly the right number), `CommitDownload` increments `download_count` + `completed_downloads` only after the stream has actually delivered the full file (no-Range GET, or `Range: bytes=0-{N-1}` covering the whole file), and `CancelDownload` releases the slot without crediting on partial ranges, decryption errors, or client disconnects. A new background reaper releases stale reservations after a configurable TTL (`DOWNLOAD_RESERVATION_TTL`, default `30m`) so a process crash mid-stream doesn't permanently lock a `max_downloads=1` file. Schema: one new column `files.in_flight_reservations` (SQLite + PostgreSQL) and one new table `download_reservations`; both migrations are non-blocking. The reaper computes its cutoff DB-side (`NOW() - ttl` in PostgreSQL, `datetime('now', '-N seconds')` in SQLite) rather than from the application clock, so wall-clock skew or NTP step events on the host can't mass-reap legitimate in-flight reservations. The `CommitDownload` reaped-recovery branch enforces the same `(download_count + in_flight_reservations) < max_downloads` invariant as `ReserveDownload`, so a late-arriving replay can't jump past a live concurrent reservation and over-count. As a separate defence-in-depth layer on top of the per-IP rate limiter, a new `MAX_INFLIGHT_PER_IP_PER_FILE` cap (default 3, 0 disables) bounds the number of concurrent reservations a single client IP can hold against a single file — blocking a Slowloris-style attacker from burst-holding many slots inside their hourly download budget. Behavioural change: partial-range HTTP requests no longer increment `download_count` — the counter now matches `completed_downloads` semantics (only successful full-file deliveries count). Closes Wave 2 of the security-hardening initiative.
